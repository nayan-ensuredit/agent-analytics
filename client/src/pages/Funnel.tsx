...
export default function Funnel() {
  const { data: conversion, loading: funnelLoading } = useFilteredApi<
    ConversionRow[]
  >("/api/funnel/conversion", 300000);
  const { data: byProduct, loading: prodLoading } = useFilteredApi<
    ProductConvRow[]
  >("/api/funnel/by-product", 300000);
  const { data: stuckQuoters, loading: stuckLoading } = useFilteredApi<
    StuckQuoterRow[]
  >("/api/funnel/stuck-quoters", 300000);
  ...
  const latestConv =
    convChartData.length > 0 ? convChartData[convChartData.length - 1] : null;
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-red-50 border border-red-200 rounded-xl">
        <p className="text-sm text-red-900">
          <strong>Quick Win:</strong> Thousands of agents are generating quotes
          but closing zero policies. This represents the #1 opportunity for
          immediate revenue uplift through targeted coaching and process
          simplification.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard
          title="Sales Funnel"
          subtitle="Quotes to Proposals to Policies with conversion rates"
          ...
          {funnelBarData.length > 0 && totals.quotes > 0 ? (
            <>
              ...
              {latestConv && (
                <div className="flex gap-3 mt-3 flex-wrap">
                  <div className="px-3 py-1.5 bg-slate-100 rounded-full text-xs font-medium text-slate-700">
                    Quote-to-Proposal:{" "}
                    {formatPercent(latestConv.quote_to_proposal_rate)}
                  </div>
                  <div className="px-3 py-1.5 bg-slate-100 rounded-full text-xs font-medium text-slate-700">
                    Proposal-to-Policy:{" "}
                    {formatPercent(latestConv.proposal_to_policy_rate)}
                  </div>
                  <div className="px-3 py-1.5 bg-slate-100 rounded-full text-xs font-medium text-slate-700">
              ...
              <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
                <strong>Insight:</strong> Each funnel drop-off represents lost
                revenue. Focus on the largest absolute drop first to maximize
        ...
        <ChartCard
          title="Conversion by Product Type"
          subtitle="Quote-to-policy rate by product"
          loading={prodLoading}
        >
          {productChartData.length > 0 ? (
            <>
                ...
                <BarChart data={productChartData}>
                  ...
                  <Tooltip
                    formatter={(
                      ...
                      if (item.dataKey === "quotes") {
                        return [formatNumber(value), "Quotes"];
                      }

                      if (item.dataKey === "proposals") {
                        return [formatNumber(value), "Proposals"];
                      }

                      if (item.dataKey === "policies") {
              ...
              <div className="mt-3 p-3 bg-amber-50 rounded-lg text-sm text-amber-800">
                <strong>Insight:</strong> Products with high quote volume but
                low conversion need pricing or UX investigation. Products with
                high conversion are ready for volume scaling.
              </div>
            </>
          ) : (
            <p className="text-sm text-slate-400 text-center py-12">
      ...
      {/* Stuck Quoters Table */}
      <ChartCard
        title="Top Agents Quoting but Not Selling"
        subtitle="Agents with quotes but zero or near-zero policies"
        ...
        {stuckQuoters && stuckQuoters.length > 0 ? (
          <>
            ...
            <div className="mt-3 p-3 bg-red-50 rounded-lg text-sm text-red-800">
              <strong>Insight:</strong> These agents are engaged (quoting) but
              failing to close. Assign sales coaches or simplify the
              proposal-to-policy flow to unlock this revenue.
            </div>
          </>
        ) : (
          <p className="text-sm text-slate-400 text-center py-12">
            No stuck quoter data available
          </p>
        )}
      </ChartCard>
    </div>
