...
export default function Insurers() {
  const { data: share, loading: shareLoading } = useFilteredApi<
    InsurerShareRow[]
  >("/api/insurers/share", 300000);
  const { data: trend, loading: trendLoading } = useFilteredApi<
    InsurerTrendRow[]
  >("/api/insurers/trend", 300000);

  /* Group into top 5 + Others for cleaner donut chart */
  const sortedShare = [...(share ?? [])].sort(
    (a, b) => Number(b.total_premium) - Number(a.total_premium),
  );
  ...
  const othersTotal = sortedShare
    .slice(5)
    .reduce((sum, d) => sum + Number(d.total_premium), 0);
  const othersPct = sortedShare
    .slice(5)
    .reduce((sum, d) => sum + Number(d.pct_share), 0);
  ...
  const { pivotedTrend, insurerKeys } = useMemo(() => {
    if (!trend || trend.length === 0)
      return { pivotedTrend: [], insurerKeys: [] };
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard
          title="Insurer Market Share"
          subtitle="Premium distribution by insurer"
                ...
                {shareChartData.slice(0, 5).map((item, i) => (
                  ...
                  >
                    ...
                    <span className="font-medium text-slate-900">
                      {formatPercent(item.pct_share)}
                    </span>
                  </div>
                ))}
              </div>
              <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
                <strong>Insight:</strong> Monitor insurer concentration.
                Over-reliance on a single insurer creates negotiation risk.
                Diversify placements to maintain competitive terms and ensure
                business continuity.
              </div>
            </>
          ) : (
            <p className="text-sm text-slate-400 text-center py-12">
              No insurer share data available
            </p>
          )}
        </ChartCard>
        ...
        <ChartCard
          title="Monthly Insurer Trend"
          subtitle="Premium trend by insurer partner"
          ...
          {pivotedTrend.length > 0 ? (
            <>
              ...
              <div className="mt-3 p-3 bg-amber-50 rounded-lg text-sm text-amber-800">
                <strong>Insight:</strong> Look for insurer trend divergence.
                Declining insurer volumes may indicate pricing issues, claim
                settlement problems, or shifting agent preferences. Engage
                insurer partners proactively.
              </div>
            </>
          ) : (
            <p className="text-sm text-slate-400 text-center py-12">
              No insurer trend data available
            </p>
          )}
        </ChartCard>
      </div>
