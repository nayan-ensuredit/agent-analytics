...
export default function Operations() {
  const { data: today, loading: todayLoading } = useFilteredApi<TodayData>(
    "/api/operations/today",
    60000,
  );
  const { data: weekComp, loading: weekLoading } = useFilteredApi<WeekData>(
    "/api/operations/week-comparison",
    300000,
  );
  const { data: leaderboard, loading: leadLoading } = useFilteredApi<
    LeaderboardRow[]
  >("/api/operations/leaderboard", 300000);
  ...
  const leaderColumns: Column[] = [
    {
      ...
      format: (v) => {
        const rank = toNum(v);
        if (rank === 1)
          return <span className="text-amber-500 font-bold">1</span>;
        if (rank === 2)
          return <span className="text-slate-400 font-bold">2</span>;
        if (rank === 3)
          return <span className="text-amber-700 font-bold">3</span>;
  ...
  const policyDelta = today
    ? toNum(today.policies_pct_of_avg) - 100
    : undefined;
  const premiumDelta = today
    ? toNum(today.premium_pct_of_avg) - 100
    : undefined;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          ...
          subtitle={
            todayLoading
              ? ""
              : `Avg: ${formatNumber(today?.avg30_policies)}/day`
          }
          delta={policyDelta}
          deltaLabel="vs avg"
        ...
        <KpiCard
          ...
          subtitle={
            todayLoading ? "" : `Avg: ${formatINR(today?.avg30_premium)}/day`
          }
          delta={premiumDelta}
          deltaLabel="vs avg"
        ...
        <KpiCard
          ...
          subtitle={
            todayLoading ? "" : `Avg: ${formatNumber(today?.avg30_agents)}/day`
          }
          delta={agentDelta}
          deltaLabel="vs avg"
        ...
        <KpiCard
          ...
          subtitle={
            todayLoading ? "" : `Avg: ${formatNumber(today?.avg30_quotes)}/day`
          }
          delta={quoteDelta}
          deltaLabel="vs avg"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard
          ...
          subtitle="Key metrics compared side-by-side"
          loading={weekLoading}
        >
          {weekChartData.length > 0 ? (
            <>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {weekChartData.map((item) => {
                  ...
                  return (
                    <div
                      key={item.metric}
                      className={`px-3 py-1.5 rounded-full text-xs font-medium ${isPositive ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"}`}
                    >
              ...
              <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
                <strong>Insight:</strong> Week-over-week trends reveal
                short-term momentum. Consistent weekly declines warrant
                immediate operational review. Focus on metrics declining for 2+
                consecutive weeks.
              </div>
            </>
          ) : (
            <p className="text-sm text-slate-400 text-center py-12">
              No week comparison data available
            </p>
          )}
        </ChartCard>
        ...
        {/* Leaderboard */}
        <ChartCard
          title="Top 20 Agents This Month"
          subtitle="Agent performance leaderboard"
          loading={leadLoading}
        >
          {rankedLeaderboard.length > 0 ? (
            <>
              ...
              <div className="mt-3 p-3 bg-emerald-50 rounded-lg text-sm text-emerald-800">
                <strong>Insight:</strong> Top performers set the benchmark.
                Study their workflows and replicate successful patterns.
                Publicly recognize leaders to drive healthy competition and
                motivate mid-tier agents.
              </div>
            </>
          ) : (
            <p className="text-sm text-slate-400 text-center py-12">
