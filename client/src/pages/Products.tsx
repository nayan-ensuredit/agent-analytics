...
export default function Products() {
  const { data: mix, loading: mixLoading } = useFilteredApi<ProductMixRow[]>(
    "/api/products/mix",
    300000,
  );
  const { data: trend, loading: trendLoading } = useFilteredApi<
    ProductTrendRow[]
  >("/api/products/trend", 300000);
  const { data: bizType, loading: bizLoading } = useFilteredApi<
    BusinessTypeRow[]
  >("/api/products/business-type", 300000);
  ...
  const { pivotedTrend, productKeys } = useMemo(() => {
    if (!trend || trend.length === 0)
      return { pivotedTrend: [], productKeys: [] };
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
        <p className="text-sm text-amber-900">
          <strong>Key Finding:</strong> The product mix is heavily skewed
          towards motor insurance. Health insurance has significantly higher
          ticket sizes but minimal volume. Diversifying into health represents a
          major premium growth opportunity.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard
          title="Product Mix by Premium"
          subtitle="Premium distribution across products"
          loading={mixLoading}
        >
          {mixChartData.length > 0 ? (
            <>
              ...
              <div className="mt-4">
                <DataTable
                  columns={mixColumns}
                  data={mixChartData as unknown as Record<string, unknown>[]}
                  pageSize={10}
                />
              </div>
              <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
                <strong>Insight:</strong> Diversification is critical.
                Over-reliance on a single product line creates vulnerability to
                regulatory or market changes. Target 20%+ share for the second
                product line.
              </div>
            </>
          ) : (
        ...
        <ChartCard
          ...
          subtitle="Premium trend by product type"
          ...
          {pivotedTrend.length > 0 ? (
            <>
              ...
              <div className="mt-3 p-3 bg-emerald-50 rounded-lg text-sm text-emerald-800">
                <strong>Insight:</strong> Watch for declining product lines. If
                a product consistently shrinks while others grow, investigate
                competitive pricing or distribution channel issues.
              </div>
            </>
          ) : (
            <p className="text-sm text-slate-400 text-center py-12">
      ...
      {/* Business Type Mix */}
      <ChartCard
        title="New vs Renewal vs Rollover"
        subtitle="Monthly business type breakdown"
        loading={bizLoading}
      >
        {pivotedBiz.length > 0 ? (
          <>
            ...
            <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
              <strong>Insight:</strong> A healthy business should have growing
              new business AND stable renewals. If renewals are declining,
              retention issues need immediate attention. High rollover
              percentage may indicate limited net new customer acquisition.
            </div>
          </>
        ) : (
          <p className="text-sm text-slate-400 text-center py-12">
