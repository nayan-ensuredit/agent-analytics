...
export default function Executive() {
  const { data: kpi, loading: kpiLoading } = useFilteredApi<KpiData>(
    "/api/executive/kpis",
    300000,
  );
  const { data: growth, loading: growthLoading } = useFilteredApi<
    GrowthPoint[]
  >("/api/executive/growth", 300000);
  const { data: concentration, loading: concLoading } =
    useFilteredApi<ConcentrationData>("/api/executive/concentration", 300000);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard
          ...
          subtitle="Policies and premium over time"
          ...
          {growthChartData.length > 0 ? (
            <>
                ...
                <LineChart data={growthChartData}>
                  ...
                  <Tooltip
                    formatter={(value: unknown, name?: string) => {
                      if (name === "Premium") {
                        return inrTooltipFormatter(value);
                      }
              ...
              <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
                <strong>Insight:</strong> Track monthly policy and premium
                trajectories to identify seasonal patterns and growth momentum.
                Sustained upward trend indicates healthy business growth.
              </div>
            </>
          ) : (
          ...
          {concentrationChartData.length > 0 ? (
            <>
              ...
              <div className="mt-3 p-3 bg-amber-50 rounded-lg text-sm text-amber-800">
                <strong>Insight:</strong> High revenue concentration in few
                entities is a risk. Diversify distribution to reduce dependency
                on top performers.
              </div>
            </>
          ) : (
