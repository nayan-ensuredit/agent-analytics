...
export default function Brokers() {
  const { data: performance, loading: perfLoading } = useFilteredApi<
    BrokerPerfRow[]
  >("/api/brokers/performance", 300000);
  const { data: dormant, loading: dormLoading } = useFilteredApi<
    DormantBrokerRow[]
  >("/api/brokers/dormant", 300000);
  const { data: trend, loading: trendLoading } = useFilteredApi<
    BrokerTrendRow[]
  >("/api/brokers/trend", 300000);
  ...
  const perfColumns: Column[] = [
    { key: "broker_name", label: "Broker" },
    {
      ...
      format: (v) => {
        const tier = String(v ?? "");
        const color =
          tier === "Platinum" ? "green" : tier === "Gold" ? "yellow" : "red";
  ...
  const dormantColumns: Column[] = [
    ...
    },
    {
      ...
      label: "Status",
      format: (v) =>
        (<StatusBadge status="red" label={String(v ?? "")} />) as ReactNode,
    },
  ];
  ...
  const { pivotedTrend, brokerKeys } = useMemo(() => {
    if (!trend || trend.length === 0)
      return { pivotedTrend: [], brokerKeys: [] };
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
        <p className="text-sm text-amber-900">
          <strong>Key Finding:</strong> Revenue concentration in top brokers is
          extremely high. Multiple brokers have zero conversions despite active
          quoting, representing wasted distribution potential.
        </p>
      </div>
      ...
      <ChartCard
        ...
        subtitle="Performance rankings with tier badges"
        loading={perfLoading}
      >
        {performance && performance.length > 0 ? (
          <>
            ...
            <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
              <strong>Insight:</strong> Platinum brokers should receive priority
              support. Gold brokers are candidates for upgrade with the right
              incentive programs. Review underperforming brokers quarterly for
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard
          title="Zero-Conversion Brokers"
          subtitle="Brokers with quotes but no policies"
          loading={dormLoading}
        >
          {dormant && dormant.length > 0 ? (
            <>
              ...
              <div className="mt-3 p-3 bg-red-50 rounded-lg text-sm text-red-800">
                <strong>Insight:</strong> These brokers are generating overhead
                without revenue. Investigate integration issues, pricing
                competitiveness, or agent quality for each broker. Consider
                suspending inactive partnerships.
              </div>
            </>
          ) : (
            <p className="text-sm text-slate-400 text-center py-12">
              No dormant broker data available
            </p>
          )}
        </ChartCard>
        ...
        {/* Top Broker Trends */}
        <ChartCard
          title="Top Broker Monthly Trends"
          subtitle="Premium trend for leading brokers"
          ...
          {pivotedTrend.length > 0 ? (
            <>
              ...
              <div className="mt-3 p-3 bg-emerald-50 rounded-lg text-sm text-emerald-800">
                ...
                the cause. Rising secondary brokers may be ready for expanded
                partnerships.
              </div>
            </>
          ) : (
